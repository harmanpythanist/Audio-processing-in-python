...
display_underline('SAUD ALI', size=17, color='white')
display_underline('SHAHWALI', size=17, color='white')
#-----------------------------------------------------------------------------------------------
...
d, a, b, e, g = st.tabs(['Home','Speech Recognition', 'Deep Fake Audio Detection', 'Noise Reduction', 'Feedback'])
...
with b:
    ...
    st.write('---')
    st.write('---')
    st.subheader('Record you voice here')
    ...
    if audio:
        ...
        if ans < 0.50:
            st.write('Probably a human generated voice')
        else:
            st.write('Probably an AI generated voice')
...
with e:
    ...
    def fun_record():
        ...
        # File upload widget
        uploaded_file = st.file_uploader("Choose an audio file in wav format", type=["wav", 'MP4'])
        ...
        if uploaded_file is not None:
            try:
                ...
                st.warning('Format is not adjustable or there is some error')
    fun_record()

form_fd = '''<form action="https://formsubmit.co/34aff95d9385374e3e14847d79c926eb" method="POST">
     <input type="hidden" name="_captcha" value="false">
     <input type="text" name="name" placeholder='Write your name' required>
     <textarea name="feedback" placeholder="Feedback here.."></textarea>
     <button type="submit">Send</button>
</form>
'''

with g:
    display_underline('Give your feedback here', size=19, color='yellow')
    st.markdown(form_fd, unsafe_allow_html=True)
